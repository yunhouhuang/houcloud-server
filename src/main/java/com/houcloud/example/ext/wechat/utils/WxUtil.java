...
import com.houcloud.example.utils.AESUtil;
import com.houcloud.example.utils.WxPKCS7Encoder;
...
public class WxUtil {
    ...
    public static JSONObject checkResult(JSONObject result){
        if(ObjectUtil.isNull(result)){
            throw BusinessException.exception("微信平台接口异常，没任何数据返回！");
        }
        if(result.containsKey("errcode") && "0".equals(result.getString("errcode"))){
            return result;
        }
        if(result.containsKey("errmsg")){
            throw BusinessException.exception("微信接口调用失败：" + result.getString("errcode") + result.getString("errmsg"));
        }
        return result;
    }
